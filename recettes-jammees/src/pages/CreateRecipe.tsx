...
/**
 * Fonction qui affiche le formulaire de création d'une recette
 * @returns le formulaire de création
 */
export default function CreateRecipe() {
    ...
    const updateStep = (stepNew: Step, indexNew: number) => {
        if (steps != undefined) {
            const stepsDummy: Array<Step> | undefined = steps.map((step, index) => {
                if (index === indexNew) {
                    stepNew.position = index + 1 // met aussi à jour l'ordre
                    return stepNew
                } else {
                    step.position = index + 1 // met aussi à jour l'ordre
    ...
    const handleCreateRecipe = () => {
        ...
        if (handleValidateName() && handleValidateCategory() && handleValidateDescription()
            ...
            Api.post("/api/recipe/createRecipe", formData).then((_) => {
                // retour utilisateur
                setColor("success");
                setIcon(<CheckCircleOutline fontSize='inherit' />)
                setInfoMessage("La création de la recette a réussit. ")
                setInfo(true)
                setLoading(false)
                ...
                setInfoMessage("La création de la recette a échoué. ")
                setInfo(true)
                setLoading(false)
            })
    ...
    return (
        <>
            ...
            <Container>
                ...
                <Typography variant="h2">Créer une recette</Typography>
                <Container>
                ...
                <Container>
                    ...
                    <Button
                        variant="contained"
                        color="secondary"
                        onClick={handleCreateRecipe}>Créer</Button>
                </Container>
            </Container>
        </>
    );
